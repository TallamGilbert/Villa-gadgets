...
[villagadgets.vercel.app](https://villagadgets.vercel.app) _(update with your URL)_
...
1. Customer fills delivery details → order saved to DB as `pending`
2. Paystack popup opens (M-Pesa or card)
3. On success → order status updated to `paid` automatically
4. On cancel → order stays as `pending` (visible in admin as abandoned)
